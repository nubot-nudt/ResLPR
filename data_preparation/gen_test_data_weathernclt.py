...
def process_scan(scan_path, dst_folder, clean_flag, severity):
    ...
    :param clean_flag: The original point cloud of NCLT or the corrupted point cloud
    ...
    if clean_flag:
        ...
        file_name_range_and_intensity = os.path.join(path_range_and_intensity, f"{idx}.npy")
    else:
...
def gen_depth_intensity_data(scan_folder, dst_folder, clean_flag, corruption_type):
    ...
    corruption_path = {'snow': '2012-02-12_vel', 'fog': '2012-06-15_vel', 'rain': '2012-11-16_vel'}

    if clean_flag:
        scan_path = os.path.join(scan_folder, 'velodyne_sync')
        folders = [scan_path]
    else:
        scan_path_light = os.path.join(scan_folder, 'light', corruption_path[corruption_type], f'{corruption_type}_velodyne')
        scan_path_mod = os.path.join(scan_folder, 'mod', corruption_path[corruption_type], f'{corruption_type}_velodyne')
        scan_path_heavy = os.path.join(scan_folder, 'heavy', corruption_path[corruption_type], f'{corruption_type}_velodyne')

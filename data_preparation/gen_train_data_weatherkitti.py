...
def process_scan(scan_path, dst_folder, seq):
    """
    ...
    file_name_range_and_intensity = os.path.join(dst_folder,  f"{save_int_inpath}_{str(idx).zfill(6)}.npy")
...
def gen_depth_intensity_data(scan_folder, dst_folder, clean_flag, corruption_type):
    ...
    for i in range(3, 11):
        ...
        if clean_flag:
            ...
            sampled_file_paths = get_file_paths(scan_path)
        else:
            ...
            #  Randomly select point cloud paths of different severity levels in proportion.
            folders = [scan_path_light, scan_path_mod, scan_path_heavy]
            sampled_file_paths = sample_files_from_folders(folders)
        ...
        with Pool() as pool:
            pool.starmap(process_scan, [(path, dst_folder, i) for path in sampled_file_paths])
